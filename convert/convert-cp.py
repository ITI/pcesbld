...
class SrvReq:
    def __init__(self, row):
        ...
        try:
            ...
            self.init['srvCP'] =  row[5] 
            self.init['srvLabel'] = row[6]
            self.init['srvOp'] = row[7] 
            self.init['msg2mc'] = {} 
        except:
    ...
    def __init__(self, row):
        ...
        try:
            self.init = {'timingcode': {}, 
                         'accelname': row[5], 'trace': cnvrtBool(row[6]), 'msg2mc': {}}
        except:
    ...
    def __init__(self, row):
        ...
        try:
            self.init = {'carried': cnvrtBool(row[3]), 
                         'xcp': row[4], 'xlabel': row[5], 
                        'trace': cnvrtBool(row[6]), 'msg2mc': {} }
        except:
...
def main():
    ...
    cmpptn_output_file = os.path.join(resultsDir, args.cmpptn_output)
    cpInit_output_file = os.path.join(resultsDir, args.cpInit_output)
            ...
            if initializations:
                ...
                match className:
                    ...
                    case 'open':
                        if cmpPtnName not in initClassDict:
                           initClassDict[cmpPtnName] = {}
                        initClassDict[cmpPtnName][funcLabel] = Open(row)
