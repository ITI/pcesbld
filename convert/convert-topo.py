...
class OpenConnection:
    def __init__(self, row):
        self.dev = row[0]
        self.network = row[1]
        self.mediatype = row[2]

    def validate(self):
        msgs = []
        if not validDev(self.dev):
            msg = "open connection specifices device {} which is not defined".format(self.dev)
            msgs.append(msg)

        if self.network not in networkNames:
            msg = 'open connection from {} specifies facing unknown network {}'.format(self.dev, self.network)
            msgs.append(msg)
   
        if self.mediatype not in ('wired', 'wireless'):
            msg = 'open connection from {} specifies unknown network media type {}'.format(self.dev, self.mediatype)
            msgs.append(msg)
...
def validateConnections():
    ...
    for conn in openConnList:
        valid, msg = conn.validate()
        if not valid:
            msgs.append(msg)

    for conn in directConnList:
...
def createLinks():
    for net in networkList:
        for endpt in net.endpts:
            # look at each of the endpt interfaces for an open interface
...
def main():
    global endptList, networkList, switchList, routerList, directConnList
    ...
    topo_results_file = os.path.join(resultsDir, args.topo_results)
    ...
    with open(csv_input_file, newline='') as rf:
        ...
        for raw in csvrdr:
            ...
            matchCode = "None"
            rowTypes = ('Networks', 'Switches', 'Routers', 'Endpoints', 'Direct-Connections', 'Open-Connections')
            ...
            if directConn:
                directConnList.append(DirectConnection(row))
            continue

            if openConn:
                openConnList.append(OpenConnection(row))
            continue
    ...
    # create interfaces for direct connections
    for conn in directConnList:
        conn.createIntrfcs()

    # create interfaces for open connections
    for conn in openConnList:
        conn.createIntrfcs()
